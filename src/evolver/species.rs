...
use crate::evaluator::{grn_genome_evaluator::GrnGenomeEvaluator, grn_sine_evaluator::SineEvaluator, grn_genome_evaluator::defaultEvaluator};
...
struct Species {
    ...
    // evaluator : Option<&'r mut dyn GrnGenomeEvaluator>, 
    evaluator : Box<dyn GrnGenomeEvaluator>,
    fitness_sum: f64,
...
impl Species {
    ...
    pub fn with_parameters(representative : GrnGenome, speciation_threshold : f64, add_repr : bool, an_evaluator : Box<dyn GrnGenomeEvaluator>) -> Species{
        unsafe {
    ...
    pub fn add_genome(&mut self, a_genome : GrnGenome , test_species : bool)  -> bool {
        ...
        if !test_species || self.representative_genome.as_ref().unwrap().distance_to(&a_genome, self.use_dynamics_genome_for_distance)<= self.speciation_threshold {
            if !a_genome.has_been_evaluated() {
                self.evaluator.as_ref().evaluate(&a_genome);
            }
	...
	pub fn recompute_fitness(&self) {
	    for k in 0..self.genomes.len() {
            ...
            self.evaluator.evaluate( self.genomes.get(k).unwrap() );
